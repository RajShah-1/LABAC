...
public class Bswabe {
    ...
    private static void fillPolicy(BswabePolicy p,
                                   BswabePub pub,
                                   Element e0)
            throws NoSuchAlgorithmException {
        int i;
    ...
    private static BswabePolynomial randPoly(int deg,
                                             Element zeroVal) {
        int i;
            ...
            if (!tok.contains("of")) {
                ...
                if (k < 1) {
                    System.out.println("error parsing " + s
                            + ": trivially satisfied operator " + token);
                    return null;
                } else if (k > n) {
                    System.out.println("error parsing " + s
                            + ": unsatisfiable operator " + token);
                    return null;
                } else if (n == 1) {
                    System.out.println("error parsing " + s
                            + ": indentity operator " + token);
                    return null;
                } else if (n > stack.size()) {
                    System.out.println("error parsing " + s
                            + ": stack underflow at " + token);
                    return null;
                }
    ...
    public static void elementFromElement(Element in, Element out)
        ...
        out.setFromHash(digest, 0, digest.length);
    }
